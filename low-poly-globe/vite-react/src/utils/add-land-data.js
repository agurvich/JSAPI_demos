...
const random = new seedrandom("lowpoly");

async function addLandData(meshLandLayer) {
    ...
    for (let { properties, geometry } of landPoints.features) {
        ...
        if (!steinerPoints[fid]) steinerPoints[fid] = [];
    ...
    for (let feature of land.features) {
        ...
        const wallGeometry = getExtrusionWallGeometry(geometry);
...
function generateTIN(polygon, innerPoints) {
    ...
    if (steinerPoints) sweepContext.addPoints(steinerPoints);
